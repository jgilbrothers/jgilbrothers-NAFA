...
export function extractLegalCandidates(documentId: string, pageTexts: string[]): LegalCandidate[] {
  ...
  pageTexts.forEach((text, pageIndex) => patterns.forEach(pattern => {
    for (const match of text.matchAll(new RegExp(pattern.expression.source, `${pattern.expression.flags.includes('g') ? pattern.expression.flags : `${pattern.expression.flags}g`}`))) {
      ...
      candidates.push({ id: `LEGAL-${documentId}-${pageIndex + 1}-${candidates.length + 1}`, documentId, kind: pattern.kind, field: pattern.field, value, sourcePage: pageIndex + 1, sourceExcerpt: match[0].trim(), confidence: 0.65, verificationStatus: 'needs_review' });
    }
  }));
  return candidates;
}
